...
export default function HeaderMenu({ data }: HeaderMenuProps) {
  if (!data) {
    console.error('HeaderMenu received null data');
        ...
        <Container className='h-full'>
          <div className='flex h-full flex-col'>
            ...
            <div className='flex flex-col items-center justify-center gap-8 md:gap-12'>
              {Array.isArray(mainNav) && mainNav.length > 0
                ? mainNav.map(
                    (item, index) =>
                      item && (
                        <Link
            ...
            <div className='flex flex-1 flex-col justify-end pb-10 md:pb-14'>
              <div className='grid grid-cols-2 gap-x-7 gap-y-5 md:flex md:flex-row md:justify-center md:gap-7'>
                {Array.isArray(secondaryNav) && secondaryNav.length > 0
                  ? secondaryNav.map(
                      (item, index) =>
                        item && (
                          <Link
